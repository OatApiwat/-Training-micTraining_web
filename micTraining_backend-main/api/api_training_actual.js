...
const training_actual_table = require("../model/training_actual");
...
router.post("/calendar_monthly_actual", async (req, res) => {
  // console.log(isLeap(req.body.year_no));
  var leap_condition = "";
  ...
  try {
    let result = await training_actual_table.sequelize.query(
      `with year_table as (
          SELECT ` +
        leap_condition +
...
router.post("/check_actual", async (req, res) => {
  try {
    let result = await training_actual_table.findOne({
...
router.post("/delete", async (req, res) => {
  ...
  try {
    let result = await training_actual_table.destroy({
...
router.post("/add", async (req, res) => {
  try {
    let insert_result = await training_actual_table.create(req.body); //await คือรอให้ส่ง ข้อมูลก่อนจึงตอบ
